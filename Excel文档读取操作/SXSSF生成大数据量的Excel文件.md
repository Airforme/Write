...
import org.apache.poi.xssf.streaming.SXSSFWorkbook;
   public static void main(String[] args) throws Throwable {
       SXSSFWorkbook wb = new SXSSFWorkbook(100); // 内存中保留100行数据,如果超出将刷新到磁盘中
...
下一个示例关闭自动刷新（windowSize = -1），并且代码手动控制如何将部分数据写入磁盘
```java
import org.apache.poi.ss.usermodel.Cell;
import org.apache.poi.ss.usermodel.Row;
import org.apache.poi.ss.usermodel.Sheet;
...
import org.apache.poi.xssf.streaming.SXSSFWorkbook;
   public static void main(String[] args) throws Throwable {
       SXSSFWorkbook wb = new SXSSFWorkbook(-1); // 关闭自动刷新数据,将数据保存在内存中
       Sheet sh = wb.createSheet();
       for(int rownum = 0; rownum < 1000; rownum++){
           Row row = sh.createRow(rownum);
           for(int cellnum = 0; cellnum < 10; cellnum++){
          ...
          // 手动控制将数据刷新到磁盘中
          if(rownum % 100 == 0) {
               ((SXSSFSheet)sh).flushRows(100); // 保留最后的100行,将其他的数据刷新到磁盘中
